...
In Figure~\ref{fig:functions:to-mc}, we give the functions that will be used
to transition from a Shelley chain state into a chain state that supports MC.
The only part of the state affected by the transition is the UTxO. For ease of
...
%%
%% Figure - Shelley to MC Transition
%%
\begin{figure}[htb]
  \begin{align*}
      & \fun{mkUTxO} ~\in~ \Slot \to \ShelleyUTxO  \to \UTxO  \\
      & \fun{mkUTxO}~s~\var{utxo} ~=~ \{~ \var{txin} \mapsto (a,\fun{coinToValue}~c) ~\vert~
      \var{txin} \mapsto \var{(a,c)}\in ~\var{utxo}~\} \\
      & \text{make UTxO with MC}
      \nextdef
      & \fun{toMC} \in ~ \ShelleyChainState \to \ChainState \\
      & \fun{toMC}~cs :
      & ~~~~ cs_{utxo} = \fun{mkUTxO}~\var{utxo} \\
      & \text{transform Shelley chain state to a MC-supporting state}
  \end{align*}
  \caption{Shelley to MC Chain State Transtition}
