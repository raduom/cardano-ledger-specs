...
\begin{figure*}[htb]
  \begin{align*}
    ...
    %
    & \fun{evalMPSScript}  ~(\type{JustMSig}~s)~\var{cid}~\var{slot}~\var{vhks}~\var{val} ~\var{txb}~\var{txouts} \\
    &~~~~ =~ \fun{evalMultiSigScript}~s~\var{vhks} \\
    %
    & \fun{evalMPSScript}
     ~(\type{RealizeTrancheTokens}~\var{tts})~\var{cid}~
     \var{slot}~\var{vhks}~\var{val}
     ~\var{txb}~\var{txouts}
      \\
    &~~~~ =~\forall~\var{cid}~ \mapsto ~tkns ~\in~ \var{val}~:~ \\
    &~~~~ \forall~ t \mapsto q ~\in~\var{tkns}, \\
    &~~~~ ((t~\notin~ \dom~\var{tts})~\Rightarrow \\
    &~~~~ t~\in~\{~\fun{renameToken}~{t_t}~\vert~t_t~\in~\dom~{tts}~\} ~\wedge \\
    &~~~~ (\forall ~t_t \mapsto q_t~\in~\var{tts}, ~\fun{renameToken}~t~=~t_t~\Rightarrow \\
    &~~~~ t_t\mapsto (-q)~\in~\var{tkns}~\wedge~\vert q\vert~\leq~q_t)) \\
    &~~~~ \wedge  \\
    &~~~~ ((t~\in~ \dom~\var{tts})~\Rightarrow \\
    &~~~~ (\fun{renameToken}~{t}~\in~\dom~\var{tkns}) \\
    &~~~~ \wedge ~(\forall~\fun{renameToken}~t\mapsto q_n~\in~\var{tkns},~q_n=-q) \\
    &~~~~ \wedge ~(\forall~t\mapsto q_t~\in~\var{tts}, \vert q\vert~\leq~q_t))
    \\~\\
    %
    & \fun{evalMPSScript}
     ~(\type{TrancheTokens}~\var{tts}~\var{txin})~\var{cid}~\var{slot}~\var{vhks}~\var{val}
     ~\var{txb}~\var{txouts}
      \\
    &~~~~ =~(\var{cid}\mapsto\var{tts}~\in~\var{val})~ \wedge~(\var{txin}~\in~\fun{txins}~{txb}) \\~\\
    %
    & \fun{evalMPSScript}
     ~(\type{FreshTokens})~\var{cid}~\var{slot}~\var{vhks}~\var{val}
     ~\var{txb}~\var{txouts}
      \\
    &~~~~ =~\forall~\var{cid}~ \mapsto ~tkns ~\in~ \var{val}~:~ \\
