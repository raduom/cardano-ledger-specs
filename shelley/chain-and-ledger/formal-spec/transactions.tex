...
\item a value (this will come from the forge value of a transaction, and contains
only the tokens of the currency specific to the script)
\end{itemize}
...
\item $\type{MaxMinTimedTotal}$~:~checks that the quantity of each type of token
in the currency being forged is allowed to be forged during the specified interval.
This can be used to restrict the total amount of each type of token of this
...
\begin{figure*}[htb]
  \emph{MC Script evaluation}
  %
  \begin{align*}
    & \fun{evalMPSScript} \in\ScriptMPS\to\Slot\to\powerset\KeyHash \to
    \Value \to\Bool  \\
    %
    & \fun{evalMPSScript}  ~(\type{JustMSig}~s)~\var{slot}~\var{vhks}~\var{val}  \\
    &~~~~ =~ \fun{evalMultiSigScript}~s~\var{vhks} \\
    %
    & \fun{evalMPSScript}
     ~(\type{MaxMinTimedTotal}~\var{trs})~\var{slot}~\var{vhks}~\var{val}
      \\
    &~~~~ =~\forall~ cid~ \mapsto ~tkns ~\in~ \var{val}~:~ \\
    &~~~~ \forall t \mapsto q \in \var{tkns}, \\
    &~~~~ (t~\in~\dom~ \var{trs}) \wedge \\
    &~~~~ (\forall~ t \mapsto (s_f, s_l, min_q, max_q) \in \var{trs} \\
    &~~~~ (min_q ~\leq q~\leq ~max_q) \wedge (s_f~\leq s~\leq ~ s_l))\\
    %
    &\fun{evalMPSScript}~(\type{RequireAll}~ls)~\var{slot}~\var{vhks}~\var{val}
   \\
    &~~~~ = \forall ~s~ \in~ ls~:~\fun{evalMPSScript}~s~\var{slot}~\var{vhks}~\var{val}\\
  \end{align*}
...
In Figure~\ref{fig:defs:tx-mc-valid}, we modify this function to call the
evaluation function specific to the MPS script type being added in this spec.

Note that the value that is passed to the evaluation function of the MPS script
contains only the tokens of the currency for which the script is being validated.

\begin{figure*}[htb]
  \emph{MC Script validation}
    %
    \begin{align*}
      \fun{validateScript} & \in\Script\Slot\to\Tx\to\Bool & \text{validate
                                                        script} \\
      \fun{validateScript} & ~\var{s}~\var{slot}~\var{tx}=
                             \begin{cases}
                               \fun{evalMultiSigScript}~s~vhks & \text{if}~s \in\MSig \\
                               \fun{evalMPSScript}~\var{slot}~\var{vhks}~\var{val} &
                                \text{if}~s \in\ScriptMPS \\
                               \mathsf{False} & \text{otherwise}
                             \end{cases} \\
                           & ~~~~\where \var{vhks}\leteq \{\fun{hashKey}~vk \vert
                             vk \in \dom(\fun{txwitsVKey}~\var{tx})\} \\
                           & ~~~~~~~~ \var{val} \leteq \fun{hashScript}~s~\restrictdom~(\fun{forge}~
                              (\fun{txbody}~tx))
    \end{align*}
  \caption{Multicurrency Script Evaluation}
